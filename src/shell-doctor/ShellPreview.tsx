...
const NAV_SECRETARIA: NavGroup[] = [
  {
    ...
    items: [
      ...
      { label: 'Pacientes', href: '/doctor/sections/pacientes', icon: Users },
      { label: 'Mensagens', href: '/doctor/sections/mensagens', icon: MessageSquare },
      { label: 'Cobrança', href: '/doctor/sections/cobranca', icon: CreditCard },
      { label: 'Configurações', href: '/doctor/sections/configuracoes-secretaria', icon: SettingsIcon },
    ],
  },
]

const NAV_PACIENTE: MobileNavItem[] = [
  { label: 'Início', href: '/doctor/paciente/inicio', icon: Home },
  { label: 'Agenda', href: '/doctor/paciente/agenda', icon: Calendar },
  { label: 'Medicação', href: '/doctor/paciente/medicacao', icon: Pill },
  { label: 'Mensagens', href: '/doctor/paciente/mensagens', icon: MessageSquare },
  { label: 'Perfil', href: '/doctor/paciente/perfil', icon: UserIcon },
]
...
export default function ShellPreview() {
  ...
  return (
    <>
      ...
      `}</style>
      <div data-nymos-doctor="true" className="min-h-screen bg-slate-100 dark:bg-slate-950">
        <PersonaToggle persona={persona} onChange={setPersona} />

        {persona === 'medico' && (
          <AppShell
            navigationGroups={NAV_MEDICO}
            activeHref="/doctor/sections/inicio"
        ...
        {persona === 'secretaria' && (
          <AppShell
            ...
            activeHref="/doctor/sections/agenda"
        ...
        {persona === 'paciente' && (
          <div className="flex justify-center items-start py-10 px-4 min-h-screen">
            <MobileShell
              ...
              activeHref="/doctor/paciente/inicio"
              notificationCount={2}
              onNavigate={(href) => console.log('Navigate:', href)}
