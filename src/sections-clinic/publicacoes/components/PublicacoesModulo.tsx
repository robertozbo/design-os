...
import { validarLegenda } from './helpers'
...
/**
 * O módulo inteiro, parametrizado pelo workspace.
 *
 * É aqui que o multi-tenant deixa de ser promessa de documento: a clínica e o
 * back-office da Nymos montam esta mesma função com o seu próprio `dados`. Não existe
 * segunda implementação, e nenhum `if tenant === 'nymos'` — o que muda é a conta
 * conectada, a cota e o conselho do autor, que já vêm nos dados.
 */
export function PublicacoesModulo({ dados: base }: { dados: PublicacoesData }) {
  ...
  /** Quem está logado — é quem aprova. Vem dos dados do workspace. */
  const aprovador = base.aprovador
  ...
  const gerar = (brief: BriefValues) => {
    ...
    setTimeout(() => {
      ...
      patch(id, {
        ...
        alertas: validarLegenda(legenda, padroes.autorPadrao.conselho, contexto.evitar),
      })
