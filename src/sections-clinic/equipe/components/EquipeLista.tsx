...
import type {
  ...
  MembroEquipe,
  PapelClinic,
} from '@/../product-clinic/sections/equipe/types'

/** Rótulo da contagem por papel no card do plano — plural quando faz sentido. */
const PAPEL_CONTAGEM: Record<PapelClinic, (n: number) => string> = {
  medico: (n) => (n === 1 ? 'médico' : 'médicos'),
  recepcao: () => 'recepção',
  admin: (n) => (n === 1 ? 'admin' : 'admins'),
}

const PAPEL_LABEL: Record<PapelClinic, string> = {
  admin: 'Admin',
  medico: 'Médico',
  recepcao: 'Recepção',
}

const PAPEL_BADGE: Record<PapelClinic, string> = {
        ...
        <div className="flex flex-col items-stretch gap-2 sm:items-end">
          ...
          <div className="w-full sm:w-72">
            ...
            <div className="mt-1.5 flex items-center gap-2.5 text-[11px] text-slate-500 dark:text-slate-400">
              {(['medico', 'recepcao', 'admin'] as PapelClinic[]).map((papel) => (
