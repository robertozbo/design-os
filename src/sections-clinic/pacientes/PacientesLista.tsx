...
import data from '@/../product-clinic/sections/pacientes/data.json'
import type {
  ClinicaCtx,
  FiltroPacientes,
  PacienteClinica,
  ...
  return (
    <>
      ...
      <PacienteDrawer
        ...
        onEditar={abrirEditar}
        onAbrirProntuario={() => navigate('/clinic/sections/prontuario')}
        onAbrirAcompanhamento={() => navigate('/clinic/sections/acompanhamento')}
        onConvidar={(id) => {
          const p = pacientes.find((x) => x.id === id)
          if (!p?.email) return
        ...
        onNovaConsulta={() => {
          setAbertoId(null)
          navigate('/clinic/sections/agenda')
        }}
      />
