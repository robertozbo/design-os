...
import {
  getAllDoctorSectionIds,
  loadDoctorSectionData,
  loadDoctorScreenDesignComponent,
} from '@/lib/doctor-section-loader'
import { AppShell, type NavGroup } from '@/shell-doctor/components'

const NAV_GROUPS: NavGroup[] = [
  {
    label: 'Atendimento',
    items: [
      { label: 'Início', href: '/doctor/sections/inicio', icon: Home },
      { label: 'Agenda', href: '/doctor/sections/agenda', icon: Calendar },
      { label: 'Pacientes', href: '/doctor/sections/pacientes', icon: Users },
    ],
  },
  {
    label: 'Clínico',
    items: [
      { label: 'Atendimentos', href: '/doctor/sections/consultas', icon: ClipboardList },
      { label: 'Exames', href: '/doctor/sections/exames', icon: FlaskConical },
      { label: 'Prescrições', href: '/doctor/sections/prescricao', icon: Pill },
    ],
  },
  {
    label: 'Operacional',
    items: [
      { label: 'Mensagens', href: '/doctor/sections/mensagens', icon: MessageSquare },
      { label: 'Configurações', href: '/doctor/sections/configuracoes-medico', icon: SettingsIcon },
    ],
  },
]
...
function resolveActiveHref(sectionId: string | undefined): string {
  if (!sectionId) return '/doctor'
  if (NESTED_UNDER_PACIENTES.has(sectionId)) {
    return '/doctor/sections/pacientes'
  }
  return `/doctor/sections/${sectionId}`
}
...
export function DoctorSectionsPage() {
  const navigate = useNavigate()
  const allIds = getAllDoctorSectionIds()
  ...
  return (
    <AppShell
      ...
      onLogout={() => navigate('/')}
      onProfileClick={() => navigate('/doctor/sections/perfil')}
      onSettingsClick={() => navigate('/doctor/sections/configuracoes-medico')}
    >
        ...
        {allIds.length === 0 ? (
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 p-8 text-center">
            <p className="text-slate-500 dark:text-slate-400">
              Nenhuma section encontrada. Crie em <code>product-doctor/sections/</code>.
            </p>
          </div>
        ) : (
...
export function DoctorSectionPage() {
  ...
  useEffect(() => {
    if (!sectionId) return
    setLoading(true)
    const data = loadDoctorSectionData(sectionId)
    ...
    const loader = loadDoctorScreenDesignComponent(sectionId, target.componentName)
    if (!loader) {
      setLoading(false)
      return
    }
