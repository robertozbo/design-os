...
interface ServicoFiltersBarProps {
  filterTypes: FilterOption<FilterTypeId>[]
  selectedFilterType: FilterTypeId
  onFilterTypeChange?: (id: FilterTypeId) => void
...
export function ServicoFiltersBar({
  filterTypes,
  selectedFilterType,
  onFilterTypeChange,
  sortOptions,
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="-mx-1 flex gap-1.5 overflow-x-auto px-1 py-1.5 sm:flex-wrap sm:overflow-visible sm:py-0">
        {filterTypes.map((f) => {
          const active = f.id === selectedFilterType
          return (
            <button
              key={f.id}
              type="button"
              onClick={() => onFilterTypeChange?.(f.id)}
