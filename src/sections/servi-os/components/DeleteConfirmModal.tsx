...
import type { Servico } from '@/../product/sections/servi-os/types'
...
export function DeleteConfirmModal({ servico, onClose, onConfirm }: DeleteConfirmModalProps) {
  useEffect(() => {
    if (!servico) return
  ...
  const hasFutureAppointments = servico.linkedAppointmentsCount > 0
      ...
      >
        ...
        <div className="space-y-3 px-6 py-5">
          <p className="text-sm leading-relaxed text-slate-600 dark:text-slate-400">
            ...
            <strong className="text-slate-900 dark:text-slate-100">"{servico.name}"</strong>. Esta ação não pode ser desfeita.
          </p>
          ...
          {hasFutureAppointments && (
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 dark:border-amber-800/60 dark:bg-amber-900/20">
              <p className="flex items-start gap-2 text-xs leading-relaxed text-amber-900 dark:text-amber-200">
                ...
                <span>
                  Existem <strong>{servico.linkedAppointmentsCount} consulta{servico.linkedAppointmentsCount === 1 ? '' : 's'} agendada{servico.linkedAppointmentsCount === 1 ? '' : 's'}</strong>{' '}
                  vinculada{servico.linkedAppointmentsCount === 1 ? '' : 's'} a este serviço. Os atendimentos não serão cancelados, mas perderão a referência de tipo e preço.
                </span>
              </p>
            </div>
          )}
