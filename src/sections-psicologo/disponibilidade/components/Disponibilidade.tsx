...
const TIMELINE_INICIO = 6 // 06h
const TIMELINE_FIM = 22 // 22h
...
function computeStats(dias: DiaDisponibilidade[], slotPadrao: SlotMinutes) {
  ...
  const slotsTotais = ativos.reduce((acc, d) => {
    ...
    return (
      acc +
      d.intervalos.reduce((a, i) => a + Math.floor(intervaloDuracaoMin(i) / slotDur), 0)
    )
  }, 0)
  return {
...
export function Disponibilidade({
  ...
  const stats = useMemo(
    () => computeStats(data.dias, data.slotMinutesPadrao),
    [data.dias, data.slotMinutesPadrao],
  )
  ...
  return (
    <div className="min-h-screen bg-slate-950 pb-32">
      <div className="mx-auto max-w-5xl px-6 pt-10">
        <Header onAplicarTemplate={() => onAplicarTemplate?.('comercial')} onLimparTudo={onLimparTudo} />

        <StatsInline
          diasAtivos={stats.diasAtivos}
          horasPorSemana={stats.horasPorSemana}
          slotsTotais={stats.slotsTotais}
          slotPadrao={data.slotMinutesPadrao}
        />
        ...
        {modo === 'lista' || true ? (
          <ListaPorDia
            dias={data.dias}
            slotPadrao={data.slotMinutesPadrao}
            expandido={diaExpandido}
            menuAbertoEm={menuAbertoEm}
            onToggleExpand={(d) => setDiaExpandido(diaExpandido === d ? null : d)}
            onMenuToggle={(d) => setMenuAbertoEm(menuAbertoEm === d ? null : d)}
            onMenuClose={() => setMenuAbertoEm(null)}
            onToggleDia={onToggleDia}
            onAddIntervalo={onAddIntervalo}
            onUpdateIntervalo={onUpdateIntervalo}
        ...
        <ConfigsGlobais
          slotPadrao={data.slotMinutesPadrao}
          antecedencia={data.antecedenciaMinima}
          bloqueioEntreSessoes={data.bloqueioEntreSessoes}
          onSetSlotPadrao={onSetSlotPadrao}
          onSetAntecedencia={onSetAntecedencia}
          onToggleBloqueio={onToggleBloqueio}
        />
      </div>

      {hasChanges ? <FooterDock onSalvar={onSalvar} onDescartar={onDescartar} /> : null}
    </div>
  )
}
  ...
  return (
    <div className="flex items-start justify-between gap-6 mb-8">
      <div>
        ...
        <h1 className="mt-2 text-[28px] leading-tight font-semibold text-slate-50">
          Quando você atende
        </h1>
        <p className="mt-2 max-w-xl text-[14px] leading-relaxed text-slate-400">
          Configure os horários recorrentes da sua semana. Pacientes só veem slots dentro
          dessa grade.
        </p>
      </div>
      <div className="flex items-center gap-4 pt-2">
        <button
          onClick={onAplicarTemplate}
          className="text-[12.5px] font-medium text-teal-300 hover:text-teal-200 transition-colors"
        >
          Carregar template ↗
        </button>
        <button
...
function StatsInline({
  ...
  return (
    <div className="flex items-baseline gap-2 mb-8 text-[13px] flex-wrap">
      <Stat valor={`${diasAtivos} dias`} label="ativos" />
      <Divisor />
      <Stat valor={`${horasPorSemana}h`} label="por semana" />
      <Divisor />
      <Stat valor={`${slotsTotais} slots`} label="disponíveis" />
      <Divisor />
      <Stat valor={`${slotPadrao}min`} label="por consulta" />
    </div>
  )
}
...
function ListaPorDia({
  ...
  dias: DiaDisponibilidade[]
  slotPadrao: SlotMinutes
  expandido: DayOfWeek | null
  menuAbertoEm: DayOfWeek | null
  onToggleExpand: (d: DayOfWeek) => void
  onMenuToggle: (d: DayOfWeek) => void
  onMenuClose: () => void
  onToggleDia?: DisponibilidadeProps['onToggleDia']
  onAddIntervalo?: DisponibilidadeProps['onAddIntervalo']
  onUpdateIntervalo?: DisponibilidadeProps['onUpdateIntervalo']
  onRemoveIntervalo?: DisponibilidadeProps['onRemoveIntervalo']
  onCopiarDia?: DisponibilidadeProps['onCopiarDia']
  onLimparDia?: DisponibilidadeProps['onLimparDia']
}) {
  return (
    <div className="rounded-2xl bg-slate-900 border border-slate-800 mb-6 overflow-hidden">
      {DIAS_ORDEM.map((d, idx) => {
        const dia = dias.find((x) => x.dayOfWeek === d)!
        const aberto = expandido === d && dia.ativo
        return (
          <div
            key={d}
            className={`relative ${
          ...
          >
            <RowDia
              dia={dia}
              slotPadrao={slotPadrao}
              aberto={aberto}
              menuAberto={menuAbertoEm === d}
              onClickRow={() => dia.ativo && onToggleExpand(d)}
              onToggle={() => onToggleDia?.(d)}
              onMenuToggle={() => onMenuToggle(d)}
    ...
    <>
      ...
      <div className="absolute right-0 top-9 z-40 min-w-[220px] rounded-xl bg-slate-900 border border-slate-700 shadow-2xl shadow-black/40 py-1.5">
        <MenuItem
          ...
          onClick={() => {
            onCopiarDia?.(dia, [1, 2, 3, 4, 5].filter((d) => d !== dia) as DayOfWeek[])
            onClose()
          }}
        />
        <MenuItem
          ...
          onClick={() => {
            onCopiarDia?.(dia, DIAS_ORDEM.filter((d) => d !== dia))
            onClose()
          }}
        />
      ...
      <div className="space-y-2">
        {dia.intervalos.map((i) => (
          <IntervaloRow
            key={i.id}
            intervalo={i}
            onUpdate={(patch) => onUpdateIntervalo?.(dia.dayOfWeek, i.id, patch)}
            onRemove={() => onRemoveIntervalo?.(dia.dayOfWeek, i.id)}
          />
        ))}
        <button
          onClick={() => onAddIntervalo?.(dia.dayOfWeek)}
        ...
        <div className="pt-2 mt-2 border-t border-slate-800/60">
          <span className="text-[11px] text-slate-500">
            ...
            {dia.slotMinutesOverride ? (
              <span className="text-teal-300/80">(override)</span>
            ) : (
  ...
  return (
    <div className="flex items-center gap-3">
      <TimeInput
        ...
        onChange={(inicio) => onUpdate({ inicio })}
      />
...
// Configs globais
// ────────────────────────────────────────────────────────────────────
function ConfigsGlobais({
  slotPadrao,
  antecedencia,
  bloqueioEntreSessoes,
  onSetSlotPadrao,
  onSetAntecedencia,
  onToggleBloqueio,
}: {
  slotPadrao: SlotMinutes
  antecedencia: AntecedenciaHoras
  ...
  return (
    <div className="rounded-2xl bg-slate-900 border border-slate-800 p-5">
      ...
      <div className="space-y-5">
        <ConfigRow
          titulo="Duração padrão da consulta"
        ...
        >
          <SegmentedSmall
            ...
            ativo={String(slotPadrao)}
            onChange={(v) => onSetSlotPadrao?.(Number(v) as SlotMinutes)}
            unidade="min"
          />
        </ConfigRow>
        ...
        >
          <SegmentedSmall
            ...
            ativo={String(antecedencia)}
            onChange={(v) => onSetAntecedencia?.(Number(v) as AntecedenciaHoras)}
            unidade="h"
          />
        </ConfigRow>
        ...
        <ConfigRow
          ...
          subtitulo="Bloqueia 5min entre uma consulta e outra"
        >
          <Toggle ativo={bloqueioEntreSessoes} onToggle={() => onToggleBloqueio?.()} />
        </ConfigRow>
      </div>
    </div>
  )
  ...
  return (
    <button
      ...
      className={`relative inline-flex h-6 w-10 rounded-full border transition-colors shrink-0 ${
        ativo
          ? 'bg-teal-500/80 border-teal-400/60'
      ...
      <div className="mx-auto max-w-5xl flex items-center justify-between gap-4">
        <div className="flex items-center gap-2.5 text-slate-300">
          ...
          <span className="text-[13px]">
            <span className="font-mono font-semibold text-slate-100">3</span> alterações
            pendentes
          </span>
        </div>
