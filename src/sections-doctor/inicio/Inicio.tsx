...
import data from '@/../product-doctor/sections/inicio/data.json'
import type {
  Profissional,
  Saudacao,
  Kpi,
  ...
  AgendaItem,
  PacienteAtencao,
} from '@/../product-doctor/sections/inicio/types'
...
export default function InicioPreview() {
  ...
  const iniciarConsulta = (agendamentoId: string) => {
    ...
    navigate(`/doctor/sections/consulta?agendamento=${agendamentoId}`)
  }
  ...
  return (
    <InicioView
      ...
      onAbrirPacienteAtencao={(id) => navigate(`/doctor/sections/pacientes`)}
      onAbrirKpi={(id) => console.log('abrir KPI:', id)}
      onFiltrarAgenda={(f) => navigate(`/doctor/sections/agenda?filtro=${f}`)}
