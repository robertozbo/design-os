...
Nav items (5):
- **Início** → `/doctor/sections/inicio` — agenda do dia + alertas (mensagens não lidas, exames novos pra revisar, pacientes do dia)
- **Agenda** → `/doctor/sections/agenda` — calendário compartilhado
- **Pacientes** → `/doctor/sections/pacientes` — lista; abrir paciente revela **tabs internas** (Resumo, Prontuário, Exames, Prescrição, Consulta, Financeiro)
- **Mensagens** → `/doctor/sections/mensagens` — canal clínico (com paciente) — separado do canal admin da secretária
- **Configurações** → `/doctor/sections/configuracoes-medico` — perfil, integrações (Memed, vídeo, IA), consentimentos LGPD, audit log, convite secretária
...
Nav items (5):
- **Agenda** → `/doctor/sections/agenda` — calendário (mesma visão do médico, mas sem campo clínico)
- **Pacientes** → `/doctor/sections/pacientes` — só dados admin (nome, contato, convênio, financeiro). **Sem prontuário, exames, prescrição, consulta**.
- **Mensagens** → `/doctor/sections/mensagens` — canal admin (com paciente). Não vê canal clínico.
- **Cobrança** → `/doctor/sections/cobranca` — links PIX/cartão, recibos, histórico, tracking textual de convênio, export CSV
- **Configurações** → `/doctor/sections/configuracoes-secretaria` — perfil próprio (sem acesso a config do consultório)
...
Bottom-nav items (5):
- **Início** → `/doctor/paciente/inicio` — próxima consulta, ações rápidas (registrar peso/glicemia, conversar com médico), lembretes de medicação do dia
- **Agenda** → `/doctor/paciente/agenda` — próximas consultas (presencial + tele), histórico, agendar/cancelar
- **Diário** → `/doctor/paciente/diario` — peso, glicemia, pressão (registro + gráficos)
- **Mensagens** → `/doctor/paciente/mensagens` — 2 threads visíveis: **Clínico** (com Dr. Pedro) e **Admin** (com secretária)
- **Perfil** → `/doctor/paciente/perfil` — dados pessoais, profissionais Nymos vinculados (multi-vinculação), consentimentos LGPD, medicações ativas, exames enviados, configurações de notificação, logout
...
- **Logo Nymos com badge da vertical:** no clínico = "Clínico". Em outros módulos Nymos = "Psi", "Nutri", "Personal". Pacote `shell-doctor` define só "Clínico".
