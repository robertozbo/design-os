...
│   - src/sections-doctor/   │                 │   - PostgreSQL              │                 │   - src/sections-mobile/    │
...
**Trigger UI:**
- `src/sections-doctor/prescricao/components/PrescricaoLista.tsx` → "Nova prescrição"
- OU `src/sections-doctor/consulta/components/Consulta.tsx` → finalizar consulta com receita

**Payload (POST `/api/doctor/prescricoes`):**
...
Mesmo fluxo, com:
- `PUT /api/doctor/prescricoes/:id/cancelar` + payload `{ motivo: MotivoCancelamento }`
- `PUT /api/doctor/prescricoes/:id/ajustar` + payload com diff
...
**Trigger UI:**
- `src/sections-doctor/exames/components/ExameDetalhe.tsx` → "Solicitar novo exame"
- OU dentro da consulta (AnamneseView com bloco "Solicitar exames")

**Payload (POST `/api/doctor/exames/solicitar`):**
...
**Trigger UI:**
- `src/sections-doctor/exames/components/ExameDetalhe.tsx` mostra:
  - Imagem do exame (visualizador inline)
  - Análise IA gerada
  - Campo "Laudo médico" editável
  - CTA "Liberar pro paciente"

**Payload (PUT `/api/doctor/exames/:id/liberar`):**
...
**Trigger UI:**
- `src/sections-doctor/consulta/components/Consulta.tsx` → "Iniciar consulta"
...
**Payload (POST `/api/doctor/consultas/:id/finalizar`):**
...
**Trigger UI:**
- `src/sections-doctor/mensagens/components/MensagensInbox.tsx`
...
Exemplos:
- `GET /api/doctor/pacientes/:id/medicacao` → só retorna se `escopo.medicacao === true`
- `GET /api/doctor/pacientes/:id/exames` → só retorna se `escopo.examesLaboratoriais === true`
- `GET /api/doctor/pacientes/:id/fotos-corporais` → só retorna se `escopo.fotosCorporais === true`
...
Médico vê em `src/sections-doctor/pacientes/components/PacienteDetalhe.tsx`:
...
```
GET /api/doctor/pacientes/:id/injecoes?medicacaoId=&periodo=30d
→ RegistroInjecao[]
```
...
```
GET /api/doctor/pacientes/:id/sintomas?medicacaoId=&periodo=90d
...
2. Aparece em fila no clínico `src/sections-doctor/prescricao/` com badge urgência
...
- Todo endpoint que cruza identidade verifica vínculo: `GET /api/doctor/pacientes/:id` 403 se não vinculado
