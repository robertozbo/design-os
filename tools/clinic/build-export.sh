#!/bin/zsh
# Monta product-plan-clinic/ do zero, de forma reprodutível.
#
...
# handwritten/; o resto é gerado a partir de product-clinic/ e src/sections-clinic/.
#
# Uso:  tools/clinic/build-export.sh
set -e
AQUI="$(cd "$(dirname "$0")" && pwd)"
RAIZ="$(cd "$AQUI/../.." && pwd)"
cd "$RAIZ"
OUT=product-plan-clinic
...
# 2. shell + tokens
cp src/shell-clinic/components/*.tsx src/shell-clinic/components/index.ts $OUT/shell/components/
cp product-clinic/design-system/*.json $OUT/design-system/

# 3. módulo _contas (compartilhado por contas-receber/pagar; o `_` marca que não é section)
python3 - <<'PY'
import re, pathlib, shutil
rw = lambda c, s: re.sub(r"@/\.\./product-clinic/sections/([^/']+)/types",
        lambda m: '../types' if m.group(1)==s else f"../../{m.group(1)}/types", c)
src=pathlib.Path('src/sections-clinic/_contas/components')
dst=pathlib.Path('product-plan-clinic/sections/_contas/components')
for f in src.iterdir(): dst.joinpath(f.name).write_text(rw(f.read_text(),'_contas'))
shutil.copy('product-clinic/sections/_contas/types.ts',
            'product-plan-clinic/sections/_contas/types.ts')
PY
